...
Commake (Common Makefile) is a base template `makefile`/[`justfile`](https://github.com/casey/just) for your software project. The aim is to make it easy to check/install requirements and to run projects without having remember commands or to consult the readme.

To start a project simply run (or the `just` equivalent):
1. `$ make init` - Check that you have the projects dependencies (e.g. `docker`)
1. `$ make install` - Install any project packages (e.g. `yarn install`)
1. `$ make run` - Run the projects development environment (e.g `yarn run`)
...
Joining new software projects is difficult, you often need to be run through the setup. As you go along finding things that you need to install and doing it all manually. 

Jumping between software projects is also difficult. You come back after a few months and no longer remember how to set the project up or the commands to run it. On some projects, the commands have not been added to the README and you have to reach out for help. Other times there is neither and you have to figure it out.

This is what `Commake` aims to fix, by providing a standard template. You can run the `init`, `install` and `run` steps.
...
The idea is not that these become really fancy, often they are just one line. [For more examples](#examples).

You don't need to always do try and everything in the script. People may be using different environments or have a different setup and dealing with that can be complicated. That complexity takes time to handle so it's better to have a simple `commake` and defer to the developer.

You can can always improve the steps, the most critical thing is getting the steps in the file, so the file can act as a checklist to begin with.

This is an example `init` that checks you have yarn and a key set. It will just fail if yarn doesn't exist and the second one just requires the dev to press enter.
