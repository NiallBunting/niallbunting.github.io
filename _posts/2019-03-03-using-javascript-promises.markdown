...
Let's imagine the restaurant only has a single chef and that a chef can only do one task at once. This means he needs to cook each item in turn, which leads to the following:

![](/assets/images/promisessinglechef.jpg)
_Multiple orders being worked on by the single chef._
...
_Multiple orders being worked on by the line chefs. The yellow is where the head chef inspects the order. The green, blue and orange are line chefs that are working on this order._
...
_Multiple dishes being worked on at the same time by line chefs. The yellow is where the head chef inspects the order. The other colours are line chefs that are working on this order._

### Javascript

Now let me relate this metaphor to Javascript.

Most of the time when Javascript code is executing, it's running as a single thread, which means it can only work on one task at a time. Therefore our head chef is the call stack of tasks to execute.

Promises are the line chefs. They can wait for blocking operations (like cooking a dish) and return the result when finished. They wait on the event loop for the call stack to become empty.
...
_This diagram shows the interaction of the call stack and the event loop._

#### Code

The following code is our starting point that we can use our newly found knowledge to improve upon.

As we can see here, the chained method is the easiest way to write promises. By writing code like the below, we can easily add additional functionality by chaining onto the end of the promise chain.
...
Hopefully this post has given you some food for thought about extracting promises that could run in parallel, as it's too easy to fall into the habit of just chaining promises together and not leveraging the event loop as effectively as possible.
