...
## What is a UK National Lottery Jackpot Roll Down?
...
- I have the statistical knowledge to handle this and haven't made errors.
...
We can work out the total rolldown prize pool by assigning that percentage of the jackpot to each of the possible rolldown categories. However, to work out how much this increases each individual prize we need to estimate how many people we expect to win each prize.
...
Estimated Entries = (9,472,000 / 70) / 0.0103950104 = 13,017,234
```
...
At about 15.5M the expected return turns positive (assuming everything else stays the same). Maybe this is why they have rolldowns to stop this from happening even though it would be a good story.

This means that a rolldown (with all the assumptions) would have slightly better odds than the roulette table, though with this number of entries we should expect a 29% chance that someone wins the jackpot.
